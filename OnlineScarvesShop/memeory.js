...
function startGame() {
  ...
  for (let i = 0; i < shuffledDeck.length; i++) {
    ...
    addImage.setAttribute("alt", "image");
...
function resetEverything() {
  ...
  minutes = 0;
  timeCounter.innerHTML =
    "<i class='fa fa-hourglass-start'></i>" + " TIMP: 00:00";
...
function match() {
  /* Access the two cards in opened array and add
    ...
    */
  const audio = new Audio("audio/bravo.mp3");
  audio.play();
  setTimeout(function () {
...
function noMatch() {
  ...
  const audio = new Audio("audio/incearca.mp3");
  audio.play();
  setTimeout(function () {
...
function AddStats() {
  ...
  p[0].innerHTML =
    "DURATA JOC: " + minutes + " Minute  " + seconds + " SECUNDE";
  p[1].innerHTML = "MISCARILE TALE: " + moves;
  p[2].innerHTML = "AI CASTIGAT: " + 3 + " STELUTE";
}
...
function winGame() {
  if (matched.length === deckCards.length) {
    ...
    AddStats();
    displayModal();
    playSound('audio/felicitari.mp3')
  }
}
...
deck.addEventListener("click", function (evt) {
  if (evt.target.nodeName === "LI") {
    ...
    console.log(evt.target.nodeName + " A fost apasat");
